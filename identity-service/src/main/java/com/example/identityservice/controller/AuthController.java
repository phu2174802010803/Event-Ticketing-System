...
import com.example.identityservice.service.SendGridEmailService;
...
public class AuthController {
    ...
    public ResponseEntity<ResponseDto> registerOrganizer(
            @Valid @RequestBody OrganizerRegistrationDto organizerRegistrationDto) {
        ...
        try {
            User user = userRepository.findByUsername(loginDto.getLogin())
                    ...
                    .orElseThrow(() -> new BadCredentialsException(
                            "Không tìm thấy người dùng với thông tin đăng nhập: " + loginDto.getLogin()));
    ...
    public ResponseEntity<?> forgotPassword(@Valid @RequestBody ForgotPasswordRequest request) {
        ...
        try {
            ...
            sendGridEmailService.sendPasswordResetEmail(request.getEmail(), token);
        } catch (IOException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
